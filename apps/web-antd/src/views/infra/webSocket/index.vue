...
function handlerSend() {
  if (!sendText.value.trim()) {
    message.warning($t('infra.webSocket.messageEmpty'));
    return;
  }
...
function getMessageTypeText(type?: string) {
  switch (type) {
    case 'group': {
      return $t('infra.webSocket.broadcast');
    }
    case 'single': {
      return $t('infra.webSocket.unicast');
    }
    case 'system': {
    ...
    <div class="mt-4 flex flex-col gap-4 md:flex-row">
      ...
      <Card :bordered="false" class="w-full md:w-1/2">
        ...
        <Select
          ...
          class="mb-3 w-full"
          size="large"
          :placeholder="$t('infra.webSocket.selectRecipient')"
          :disabled="!getIsOpen"
        >
          <Select.Option key="" value="" label="所有人">
            <div class="flex items-center">
              <Avatar size="small" class="mr-2">全</Avatar>
              <span>{{ $t('infra.webSocket.allUsers') }}</span>
            </div>
          </Select.Option>
          <Select.Option
        ...
        <Input.TextArea
          ...
          allow-clear
          :placeholder="$t('infra.webSocket.enterMessage')"
        />
