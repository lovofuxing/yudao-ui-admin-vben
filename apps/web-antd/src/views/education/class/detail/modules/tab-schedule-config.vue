...
import { getClassScheduleConfigPage } from '#/api/education/class/detail';
const props = defineProps<{ classId: number }>();
const [Grid] = useVbenVxeGrid({
  formOptions: { schema: () => [] },
  gridOptions: {
    columns: () => [
      { field: 'scheduleType', title: '排课方式', minWidth: 100 },
      { field: 'weekday', title: '上课星期', minWidth: 120 },
      { field: 'timeRange', title: '时间段', minWidth: 130 },
      { field: 'teacher', title: '老师', minWidth: 100 },
    ...
    proxyConfig: {
      ajax: {
        query: async ({ page }, fv) =>
          await getClassScheduleConfigPage({
