...
const [Grid] = useVbenVxeGrid({
  formOptions: {
    schema: () => [
      { component: 'Input', fieldName: 'followType', label: '跟进方式', componentProps: { allowClear: true } },
      { component: 'Input', fieldName: 'content', label: '跟进内容', componentProps: { allowClear: true } },
    ],
  },
  gridOptions: {
    columns: () => [
    ...
    proxyConfig: {
      ajax: {
        query: async ({ page }, formValues) => {
          return await getStudentFollowPage({ studentId: props.studentId, pageNo: page.currentPage, pageSize: page.pageSize, ...formValues });
        },
      },
    },
